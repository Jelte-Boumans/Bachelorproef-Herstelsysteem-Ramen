    ...
    function update_action($view_mode, $user){
        ...
        if ($this->is_changed($view_mode,$user)){
            ...
            for($i = 0; $i < count($selectedWindows); $i++) {
                ...
                $action->set_value("win".$currentWindow."_window_saved", get_request_parameter("window_saved")[($currentWindow-1)]);
                $action->set_value("win".$currentWindow."_name_technician", get_request_parameter("name_technician")[($currentWindow-1)]);
            }
